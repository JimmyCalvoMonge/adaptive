...
def get_coefficients_cubic(Rphi, Rmu, R0, kappa, theta):
    ...
    # Returns the coefficients x3,x2,x1,x0 of the cubic
    # polynomial in the notes.
    """
    kappa := C^i/C^s
    theta := C^z/C^s
    """
...
def nmbr_rts_interval(Rphi, Rmu, val_tuple):
    ...
    for R0 in tqdm(R0s):
        ...
        values_at_start = [float(sp.Poly(pol_sturm, x).eval(0))
                           for pol_sturm in sturm_seq]
        values_at_end = [float(sp.Poly(pol_sturm, x).eval(1))
                         for pol_sturm in sturm_seq]
...
def solve_polynomial(Rphi, Rmu, R0, kappa, theta):
    ...
    resp = [(float(expr.as_real_imag()[0]), float(expr.as_real_imag()[1]))
            for expr in resp]
...
def get_convergence_point(
    ...
    non_adaptive = NonAdaptive(
        mu, gamma, beta, phi, Cs, Ci, Cz, x00, t_max, dt=0.1)
