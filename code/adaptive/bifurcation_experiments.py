...
def get_convergence_point(mu, gamma, beta, phi, kappa, theta, val_tuple):
    ...
    if not f"res_{kappa}_{theta}_{prop}_{R0}.csv" in os.listdir(adaptive_folder):
        ...
        print(f"Calculation for {(kappa, theta, prop, R0)} took {(end-start)/60} minutes. Message = {message}.")
        ...
        end_tuple = time.time()
        print(f"Tuple {(kappa, theta, prop, R0)} DONE. This took {end_tuple - start_tuple} seconds.")
